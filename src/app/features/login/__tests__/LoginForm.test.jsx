...
describe('LoginForm Component', () => {
    ...
    test('handles form submission with valid data', async () => {
        ...
        expect(localStorage.setItem).toHaveBeenCalledWith('user_data', JSON.stringify({
            name: 'Test User',
            email: 'test@example.com',
        }));
    });
    ...
    test('displays loading state during login process', async () => {
        ...
        login.mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve({ success: true }), 100)));
        render(<LoginForm />);
        // Fill in the form
        fireEvent.change(screen.getByLabelText('Username'), {
            target: { value: 'test@example.com' },
