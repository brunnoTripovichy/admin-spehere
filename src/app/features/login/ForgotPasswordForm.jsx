...
const ForgotPasswordForm = ({ className, }) => {
    ...
    return (<div className={`flex flex-col gap-4 ${className}`} data-testid="forgot-password-form">
      {error && (<div className="p-3 bg-red-100 border border-red-300 text-red-700 rounded-md text-sm">
          {error}
        </div>)}

      {!isSubmitted ? (<Form onSubmit={handleSubmit} validationSchema={schema} defaultValues={{
                email: '',
            }}>
          <TextField name="email" label={t('form.email', 'login')} placeholder={t('form.emailPlaceholder', 'login')}/>
          ...
          <Button type="submit" className="w-full" disabled={isSubmitting}>
            {isSubmitting ? (<span className="flex items-center justify-center">
              ...
              </span>) : (t('form.resetPassword', 'login'))}
          </Button>
      ...
      <div className="mt-6 text-center">
        <Link href={`/${lng}/login`} className="text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
